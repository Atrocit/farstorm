	...
	async inTransaction<P>(executor: (dbFunctions: DbFunctions<ED>) => Promise<P>, transactionConfig?: TransactionConfig<ED>): Promise<P> {
		...
		const createOutputTypeFromRawSqlType = <N extends EntityName<ED>>(entityName: N, result: RawSqlType<ED, EntityByName<ED, N>>): OutputTypeByName<ED>[N] => {
			...
			for (const relationName of Object.keys(entityDefinition.oneToOneOwned)) {
				...
				const getOneToOneRelation = async () => {
					...
					let cachedPromise = ownedRelationCache.find(entityName, relationName);
					if (cachedPromise == null) {
						cachedPromise = fetchOneToOneOwnedRelation(entityName, relationName);
						ownedRelationCache.overwrite(entityName, relationName, cachedPromise);
					}
					await cachedPromise;
			...
			for (const relationName of Object.keys(entityDefinition.manyToOne)) {
				...
				const getManyToOneRelation = async () => {
					...
					let cachedPromise = ownedRelationCache.find(entityName, relationName);
					if (cachedPromise == null) {
						cachedPromise = fetchManyToOneRelation(entityName, relationName);
						ownedRelationCache.overwrite(entityName, relationName, cachedPromise);
					}
					await cachedPromise;
			...
			for (const relationName of Object.keys(entityDefinition.oneToOneInverse)) {
				...
				const getOneToOneInverseRelation = async () => {
					...
					let cachedPromise = inverseRelationCache.find(entityName, relationName);
					if (cachedPromise == null) {
						cachedPromise = fetchOneToOneInverseRelation(entityName, relationName);
						inverseRelationCache.overwrite(entityName, relationName, cachedPromise);
					}
			...
			for (const relationName of Object.keys(entityDefinition.oneToMany)) {
				...
				const getOneToMany = async () => {
					...
					let cachedPromise = inverseRelationCache.find(entityName, relationName);
					if (cachedPromise == null) {
						const fetchedPromise = fetchOneToManyRelation(entityName, relationName) as Promise<{ ids: string[], inverseMap: Record<string, { id: string }[]> }>;
						inverseRelationCache.overwrite(entityName, relationName, fetchedPromise);
						cachedPromise = fetchedPromise;
					}
					const cached = await cachedPromise;
		...
		const updateCacheWithNewEntities = <N extends EntityName<ED>>(entityName: N, results: RawSqlType<ED, EntityByName<ED, N>>[]) => {
			...
			for (let i = 0; i < results.length; i++) {
				...
				const typeOfSave = localCache.save(entityName, id, results[i]);
		...
		const invalidateRelationCachesAfterWrite = (entityName: EntityName<ED>) => {
			// Owned relations depend on foreign keys stored on the entity that was written.
			ownedRelationCache.invalidateForEntity(entityName);

			// Inverse relations also depend on writes to the entity on the owning side.
			inverseRelationCache.invalidateForEntity(entityName);
			inverseRelationCache.invalidateByCondition((rcEntityName, rcRelationName) => {
				return this.entityDefinitions[rcEntityName].oneToOneInverse[rcRelationName]?.entity == entityName
					|| this.entityDefinitions[rcEntityName].oneToMany[rcRelationName]?.entity == entityName;
			});
		};
		...
		const saveMany = async <N extends EntityName<ED>>(entityName: N, entities: InputTypeByName<ED>[N][]): Promise<OutputTypeByName<ED>[N][]> => {
			...
			const rawEntities = await Promise.all(entities.filter(e => e != null).map(async (e: Record<string, any>) => {
				...
				for (const field of Object.keys(entityDefinition.fields)) {
					...
					if (fieldDefinition.readOnly) continue;
			...
			const entitiesToUpdate = rawEntities.filter(e => e['id'] != null && Object.keys(e).some(field => field != 'id'));
			const entitiesToRefresh = rawEntities.filter(e => e['id'] != null && Object.keys(e).every(field => field == 'id'));
			...
			if (entitiesToInsert.length > 0) {
				...
				invalidateRelationCachesAfterWrite(entityName);
			}
			...
			// An entity can contain only an ID and read-only fields. Fetch its current state instead of generating an empty update.
			if (entitiesToRefresh.length > 0) {
				const ids = entitiesToRefresh.map(entity => entity.id);
				const refreshedRows = await nativeQuery({ sql: `select * from "${tableName}" where "id" = any($1)`, params: [ ids ] });
				const refreshedRowsById = Object.fromEntries(refreshedRows.map(row => [ row.id, row ]));
				const orderedRows = ids.map(id => refreshedRowsById[id]).filter(row => row != null);
				rows.push(...orderedRows);
				updateCacheWithNewEntities(entityName, orderedRows);
			}
		...
		const deleteMany = async <N extends EntityName<ED>>(entityName: N, options: { where: WhereClause<ED, EntityByName<ED, N>> }): Promise<void> => {
			...
			if (this.auditLoggingEnabled && this.entityDefinitions[entityName].auditLogging == 'ENABLED') {
				sqlStatement = {
					sql: `
		...
		try {
			...
			throw e;
		} /* v8 ignore next -- @preserve */ finally {
...
export { type AuditLogging, defineAutogeneratedField, defineCustomField, defineEntity, defineField, defineIdField, defineReadonlyField } from './entities/BaseEntity.js';

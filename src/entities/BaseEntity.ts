...
export function defineReadonlyField<Null extends Nullable, T extends keyof typeof defaultFieldTypes>(type: T, nullable: Null): Omit<typeof defaultFieldTypes[T], 'nullableOnInput' | 'nullableOnOutput' | 'readOnly'> & { nullableOnInput: 'NULLABLE', nullableOnOutput: Null, readOnly: true } {
	return { ...defaultFieldTypes[type], nullableOnInput: 'NULLABLE', nullableOnOutput: nullable, readOnly: true } as const;
}
...
export function defineCustomField<Null extends Nullable, ToType extends (x: any) => any, FromType extends (x: any) => any>(nullable: Null, toType: ToType, fromType: FromType) {
	return { nullableOnInput: nullable, nullableOnOutput: nullable, readOnly: false, toType, fromType } as const;
}

export function defineIdField() {
	return { nullableOnInput: 'NULLABLE', nullableOnOutput: 'NOT NULL', readOnly: false, toType: (x: number) => x.toString(), fromType: (x: string) => Number(x) } as const;
}
